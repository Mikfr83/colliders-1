...
MObject SkirtBellCollider::attr_bellRadiusRamp;
...
static MMatrix createRingMatrix(const MPoint& A, const MPoint& B, const MVector& scale, double customLength = -1.0)
{
    MVector Y = B - A;
    double len = (customLength >= 0.0) ? customLength : Y.length();
    if (Y.length() > 1e-6)
    {
        Y.normalize();
    }
    ...
    MVector temp(0, 0, 1);
    if (abs(Y.x) < 1e-4 && abs(Y.y) < 1e-4)
    {
        temp = MVector(1, 0, 0);
    }
    MVector X = (temp ^ Y).normal();
...
void SkirtBellCollider::postConstructor()
{
    ...
    MRampAttribute rampAttr(thisObj, attr_bellRadiusRamp);
...
MStatus SkirtBellCollider::initialize()
{
    ...
    // Bell Radius Ramp (Curve)
    attr_bellRadiusRamp = MRampAttribute::createCurveRamp("bellRadiusRamp", "bellRadiusRamp");
    addAttribute(attr_bellRadiusRamp);
    ...
    attributeAffects(attr_bellRadiusRamp, attr_outputSurface);
...
MStatus SkirtBellCollider::compute(const MPlug& plug, MDataBlock& dataBlock)
{
    ...
    // Ramp attribute
    MRampAttribute rampAttr(thisMObject(), attr_bellRadiusRamp);
    ...
    // Get direction vector based on the local Y axis of the bell matrix
    const MVector raw_dir_vector = yaxis(inputBellMatrix);
    ...
    // Clamp ring scale for numerical stability in solver
    const double safeRingScaleX = ringScale.x < 1e-4 ? 1e-4 : ringScale.x;
    const double safeRingScaleY = ringScale.y < 1e-4 ? 1e-4 : ringScale.y;
    const double safeRingScaleZ = ringScale.z < 1e-4 ? 1e-4 : ringScale.z;
    const MVector safeRingScale(safeRingScaleX, safeRingScaleY, safeRingScaleZ);

    // Construct collision rings
    const MMatrix leftHipToKnee = createRingMatrix(LH, LK, safeRingScale, L_thigh);
    const MMatrix rightHipToKnee = createRingMatrix(RH, RK, safeRingScale, L_thigh);
    ...
    if (skirtType == 1)
    {
        ...
        leftHipToHeel = createRingMatrix(LH, LHe, safeRingScale, leftLegLen);
        rightHipToHeel = createRingMatrix(RH, RHe, safeRingScale, rightLegLen);
    }
    ...
    // Setup uKnots: clamped at start (degree 3) and end for kClosed form
    for (int j = 0; j < 3; j++)
    {
        uKnots.append(0.0);
    }
    for (int j = 1; j < bellSubdivision; j++)
    {
        uKnots.append((double)j);
    }
    for (int j = 0; j < 3; j++)
    {
        uKnots.append((double)bellSubdivision);
    }
    ...
    surfaceFn.create(
        ...
        uKnots,
        vKnots,
        3, // uDegree (cubic closed)
        1, // vDegree (linear open)
        MFnNurbsSurface::kClosed, // uForm
    ...
    MUserData* oldData)
{
    ...
    MMatrix leftHipToKnee = createRingMatrix(LH, LK, safeRingScale, leftThigh);
    MMatrix rightHipToKnee = createRingMatrix(RH, RK, safeRingScale, rightThigh);
    ...
    if (skirtType == 1)
    {
        ...
        MMatrix leftHipToHeel = createRingMatrix(LH, LHe, safeRingScale, leftLegLen);
        MMatrix rightHipToHeel = createRingMatrix(RH, RHe, safeRingScale, rightLegLen);
